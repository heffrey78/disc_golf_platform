...
const forumService = {
    ...
    createCategory: async (name: string, description: string) => {
      try {
        ...
        const response = await axios.post(`${API_URL}/forum/categories`, 
          { name, description },
    ...
    getSubforumsByCategory: async (categoryId: number, page: number = 1, limit: number = 10) => {
      try {
        const response = await axios.get(`${API_URL}/forum/categories/${categoryId}/subforums`, {
          params: { page, limit }
        });
        return response.data;
      } catch (error) {
        handleError(error as AxiosError<ErrorResponse>, 'getSubforumsByCategory');
      }
    },
    ...
    createSubforum: async (name: string, description: string, categoryId: number) => {
      try {
        const token = getAuthToken();
        const response = await axios.post(`${API_URL}/forum/subforums`, 
    ...
    createThread: async (title: string, content: string, subforumId: number) => {
      try {
        const token = getAuthToken();
        const response = await axios.post(`${API_URL}/forum/threads`, 
          { title, content, subforumId },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        return response.data;
      } catch (error) {
    ...
    updatePost: async (id: number, content: string) => {
      try {
        const token = getAuthToken();
        const response = await axios.put(`${API_URL}/forum/posts/${id}`, 
          { content },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        return response.data;
